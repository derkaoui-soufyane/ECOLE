...
import Etudiants from '../étudiants/étudiants';
import Note_ense from '../note/note_ense';
import Ense_calendrier from '../calendrier/ense_calendrier';
...
function getStoredRole() {
  return secureSession.get("codeENS") || secureLocal.get("codeENS");
}

function clearAuth() {
  ["codeENS", "codeENS2"].forEach((k) => {
...
export default function Dash() {
  ...
  function getUsername() {
    return secureSession.get("enseignant") || secureLocal.get("enseignant") || "enseignant";
  }
  function getemail() {
    return secureSession.get("codeENS2") || secureLocal.get("codeENS2") || "enseignant";
  }
  

  const enseignant = getUsername();
  ...
  useEffect(() => {
    ...
    if (role !== 'enseignant') navigate('/Connection');
  }, [navigate]);
  ...
  const filteremail = () => {
  return DataEnse
    .filter(v => v.user?.email === getemail())
    .map(v => v.id_classe);
}
 const prof = () => {
  return DataEnse
...
const etud = () => {
  const classes = filteremail(); // array
  return dataEtudiant.filter(v => classes.includes(v.id_classe));
}
        ...
        <div className="mdione-sidebar__footer">
          <div className="mdione-sidebar__user">
            <div className="mdione-sidebar__avatar">
              {getInitials(enseignant)}
            </div>
            <div className="mdione-sidebar__user-info">
              <span className="mdione-sidebar__user-name">{enseignant || 'Admin'}</span>
              <span className="mdione-sidebar__user-role">enseignant</span>
            </div>
          </div>
        ...
        <header className="mdione-topbar">
          <div className="mdione-topbar__left">
            ...
            <div className="mdione-topbar__title-block">
              ...
              <p className="mdione-topbar__greeting">
                {getGreeting()}, <strong>{enseignant || 'Admin'}</strong> 👋
              </p>
            </div>
          </div>
          ...
          <div className="mdione-topbar__right">
            <div className="mdione-topbar__user-chip">
              ...
              <span className="mdione-topbar__user-chip-name">{enseignant || 'Admin'}</span>
            </div>
          </div>
        </header>
