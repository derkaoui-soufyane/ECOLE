...
import Calend_etd from './calend_etd.jsx/calend_etd';
import Note_etd from './note_etd/note_etd';
import Parametre_etd from './parametre_etd/parametre_etd';
...
// ✅ FIXED: was reading "codeENS" (teacher key) — must read "codeETD" (student key)
function getStoredRole() {
  return secureSession.get("codeETD") || secureLocal.get("codeETD");
}

// ✅ FIXED: was clearing "codeENS" (teacher key) — must clear all "codeETD" keys
function clearAuth() {
  ["codeETD", "codeETD2", "etudiant"].forEach((k) => {
...
function LoadingScreen() {
  return (
    <div className="mdione-loading-screen">
      ...
      <p className="mdione-loading-text">Chargement de l'espace étudiant…</p>
    </div>
  );
}

// ── Main component ─────────────────────────────────────────────────────────────
export default function Dash_etd() {
  ...
  function getUsername() {
    return secureSession.get("etudiant") || secureLocal.get("etudiant") || "etudiant";
  }
  function getemail() {
    return secureSession.get("codeETD2") || secureLocal.get("codeETD2") || "etudiant";
  }

  const etudiant = getUsername();
  ...
  // ✅ FIXED: auth guard now correctly checks for "etudiant" (was checking "enseignant")
  useEffect(() => {
    const role = getStoredRole();
    if (role !== 'etudiant') navigate('/Connection');
  }, [navigate]);
    ...
    <div className="mdione-layout">
      ...
      <aside className={`mdione-sidebar${collapsed ? ' mdione-sidebar--collapsed' : ''}`}>
        ...
        <div className="mdione-sidebar__brand">
          ...
          <div className="mdione-sidebar__brand-text">
            ...
            <span className="mdione-sidebar__subtitle">Espace Étudiant</span>
          </div>
        </div>
        ...
        <div className="mdione-sidebar__footer">
          <div className="mdione-sidebar__user">
            ...
            <div className="mdione-sidebar__user-info">
              <span className="mdione-sidebar__user-name">{etudiant || 'Étudiant'}</span>
              <span className="mdione-sidebar__user-role">étudiant</span>
            </div>
          </div>
      ...
      <main className={`mdione-main${collapsed ? ' mdione-main--expanded' : ''}`}>
        ...
        <header className="mdione-topbar">
          <div className="mdione-topbar__left">
            ...
            <div className="mdione-topbar__title-block">
              ...
              <p className="mdione-topbar__greeting">
                {getGreeting()}, <strong>{etudiant || 'Étudiant'}</strong> 👋
              </p>
            </div>
          </div>
          ...
          <div className="mdione-topbar__right">
            <div className="mdione-topbar__user-chip">
              ...
              <span className="mdione-topbar__user-chip-name">{etudiant || 'Étudiant'}</span>
            </div>
          </div>
        </header>
        ...
        <div className="mdione-page-content">
          ...
          <Calend_etd  dataetd={dataetd()}
          DataSalleDispo={DataSalleDispo}
          />
          }
          {activeNav==="Note" && 
          
          <Note_etd  dataetd={dataetd()}
          dataFilieres={dataFilieres}
          />
          }
          {activeNav==="Paramètres" && 
          
          <Parametre_etd  dataetd={dataetd()}
